"""
    B-Complement
    feat_len = 1024
    min_of_n loss version
    add matching loss, shape cd loss
    put also instance_label to the network
"""
import ipdb
import os
import time
...
from data import PartNetPartDataset
import utils
...
def train(conf):
    ...
    if not conf.no_console_log:
        header = "     Time    Epoch     Dataset    Iteration    Progress(%)       LR    TransL2Loss    RotL2Loss   RotCDLoss   TotalLoss"
...
def forward(
    ...
    seq = torch.cat(batch[data_features.index("seq")], dim=0).to(conf.device)  # B x P
    ...
    # get instance label
    instance_label = torch.zeros(batch_size, num_part, num_part).to(
        conf.device
    )  # B x P x P
    for i in range(batch_size):
        num_class = [0 for i in range(60)]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--train_data_fn",
        type=str,
        help="training data file that indexs all data tuples",
    )
    parser.add_argument(
        "--val_data_fn",
        type=str,
        help="validation data file that indexs all data tuples",
    )
    ...
    parser.add_argument(
        "--data_dir", type=str, default="../../prepare_data/", help="data directory"
    )
    parser.add_argument(
        "--overwrite",
        action="store_true",
    ...
    # network settings
    parser.add_argument("--feat_len", type=int, default=128)
    ...
    os.system(
        "cp data.py models/%s.py %s %s" % (conf.model_version, __file__, conf.exp_dir)
    )
