...
def train(conf):
    ...
    training_data = PartNetDistractorDataset(training_dataset=train_dataset, distractor_dataset=train_distractor_dataset)
    ...
    val_data = PartNetDistractorDataset(training_dataset=val_dataset, distractor_dataset=val_distractor_dataset)

    utils.printout(conf.flog, str(val_data))
    val_dataloader = torch.utils.data.DataLoader(val_data, batch_size=conf.batch_size, shuffle=False,
                                                 pin_memory=True, \
    ...
    for epoch in range(conf.epochs):
        ...
        for train_batch_ind, batch in train_batches:
            ...
            total_loss, total_trans_l2_loss, total_rot_l2_loss, total_rot_cd_loss, total_shape_cd_loss = forward(
                batch=batch, data_features=data_features, network=network, conf=conf, is_val=False, \
            ...
            while val_fraction_done <= train_fraction_done and val_batch_ind + 1 < val_num_batch:
                ...
                with torch.no_grad():
                    ...
                    total_loss, total_trans_l2_loss, total_rot_l2_loss, total_rot_cd_loss, total_shape_cd_loss = forward(
                        batch=val_batch, data_features=data_features, network=network, conf=conf, is_val=True, \
...
def forward(batch, data_features, network, conf, \
    ...
    for repeat_ind in range(repeat_times):
        total_pred_part_poses = network(conf, input_part_pairs.float(), input_part_valids.float(),
                                        input_part_pcs.float(), instance_label,
                                        same_class_list)  # B x P x P, B x P, B x P x N x 3
    ...
    with torch.no_grad():
        ...
        if is_val and (not conf.no_visu) and epoch % conf.num_epoch_every_visu == conf.num_epoch_every_visu - 1:
            ...
            if batch_ind < conf.num_batch_every_visu:
                ...
                pred_pts = qrot(pred_part_poses[:, :, 3:].unsqueeze(2).repeat(1, 1, num_point, 1),
...
if __name__ == '__main__':
    ...
    parser.add_argument('--training_distractor_file', type=str, help='Path to the npy file for training')

    parser.add_argument('--validation_data_file', type=str, help='Path to the npy file for training')
    parser.add_argument('--validation_distractor_file', type=str, help='Path to the npy file for training')
    ...
    parser.add_argument('--max_distractor_num_part', type=int, default=1)
