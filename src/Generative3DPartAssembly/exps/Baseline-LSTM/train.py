...
def train(conf):
    ...
    data_features = ['part_pcs', 'part_poses', 'part_valids', 'shape_id', 'part_ids']
...
def forward(batch, data_features, network, conf, \
    ...
    part_ids = torch.cat(batch[data_features.index('part_ids')], dim=0).to(conf.device)  # B x P x 1


    repeat_times = 5
    for repeat_ind in range(repeat_times):
        # forward through the network
        pred_part_poses = network(input_part_pcs, input_part_valids).to(conf.device)     # B x P x (3 + 4)
